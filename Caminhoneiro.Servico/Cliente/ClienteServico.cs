...
namespace Caminhoneiro.Servico.Cliente
{
        ...
        public RetornoClienteModel getCliente(string clientId,string apiKey, string apiVersion,string token,string cpf)
        {
            ...
            try {
                using (var client = new HttpClient())
                {
                    ...
                    Task<HttpResponseMessage> getTask = client.GetAsync($"?cpf={cpf}");
                    getTask.Wait();
                    ...
                    ret = JsonConvert.DeserializeObject<RetornoClienteModel>(log);
